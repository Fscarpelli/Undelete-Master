...
| Privilege expansion | Protocol/native boundary | Write, arbitrary path or generic control under elevation | No mutation/path/access-mask/control field; fixed query-only IOCTLs; exactly five audited `CreateFileW` shapes; scan source alone uses `GENERIC_READ`; destination volume uses desired access `0` | Static source validation passes; built-binary import/access-mask evidence remains pending | Open |
